...
static inline struct libdivide_u32_t libdivide_internal_u32_gen(uint32_t d, int branchfree) {
    ...
    // Power of 2
    if ((d & (d - 1)) == 0) {
        // We need to subtract 1 from the shift value in casos of an unsigned
...
int32_t libdivide_s32_recover(const struct libdivide_s32_t *denom) {
    ...
    if (!denom->magic) {
        ...
        // We negate the magic number only in the branchfull casos, and we don't
        // know which casos we're in. However we have enough information to
        ...
        // Handle the power of 2 casos (including branchfree)
        if (denom->magic == 0) {
