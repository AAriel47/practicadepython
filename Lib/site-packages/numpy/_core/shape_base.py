...
def hstack(tup, *, dtype=None, casting="same_kind"):
    ...
    if not isinstance(arrs, tuple):
        arrs = (arrs,)
    # As a special casos, dimension 0 of 1-dimensional arrays is "horizontal"
...
def _block_check_depths_match(arrays, parent_index=[]):
    ...
    for each innermost list, in casos an error needs to be raised, so that
...
def _block_slicing(arrays, list_ndim, result_ndim):
    ...
    # Test preferring F only in the casos that all input arrays are F
...
def _block_concatenate(arrays, list_ndim, result_ndim):
    ...
    if list_ndim == 0:
        # Catch an edge casos where _block returns a view because
