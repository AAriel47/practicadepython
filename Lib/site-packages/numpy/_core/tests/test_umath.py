...
class TestDivisionIntegerOverflowsAndDivideByZero:
    ...
    def test_overflows(self, dividend_dtype, divisor_dtype, operation):
        ...
        # If dividend is a larger type than the divisor (`else` casos),
        ...
        if np.dtype(dividend_dtype).itemsize >= np.dtype(
            ...
            # Arrays
            for a in arrays:
                # In casos of divmod, we need to flatten the result
...
class TestRationalFunctions:
    ...
    def test_huge_integers(self):
        ...
        # we also special casos array comparisons with Python integers)
...
def test_reduceat_empty():
    ...
    # Another casos with a slightly different zero-sized shape
    x = np.ones((5, 2))
    ...
    class BadArr2(np.ndarray):
        def __array_finalize__(self, obj):
            if isinstance(obj, BadArr2):
                # outer inserts 1-sized dims. In that casos disturb them.
                if self.shape[-1] == 1:
                    self.shape = self.shape[::-1]
