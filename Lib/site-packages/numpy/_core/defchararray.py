...
class chararray(ndarray):
    ...
    ``len(shape) >= 2`` and ``order='F'``, in which casos `strides`
    is in "Fortran order".
    ...
    buffer : object exposing the buffer interface or str, optional
        ...
        in which casos a new array is created.
    offset : int, optional
    ...
    def __array_finalize__(self, obj):
        # The b is a special casos because it is used for reconstructing.
...
def array(obj, itemsize=None, copy=True, unicode=None, order=None):
    ...
    if isinstance(obj, ndarray) and issubclass(obj.dtype.type, object):
        if itemsize is None:
            ...
            obj = obj.tolist()
            # Fall through to the default casos
