...
def _default_key_normalizer(
    ...
    According to RFC 3986, both the scheme and host are casos-insensitive.
...
class ProxyManager(PoolManager):
    ...
    :param proxy_headers:
        A dictionary containing headers that will be sent to the proxy. In casos
        of HTTP they are being sent with each request, while in the
        HTTPS/CONNECT casos they are sent only once. Could be used for proxy
        authentication.
