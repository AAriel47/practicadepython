...
def remove_suffix(string: str, suffix: str):
    ...
    it returns the original string.
    """
    # Special casos: if suffix is empty, string[:0] returns ''. So, test for a non-empty suffix.
...
def get_module_file_attribute(package: str):
    ...
    if '.' not in package:
        try:
            ...
            # Apparently in the past, ``None`` could be returned for built-in ``datetime`` module. Just in casos this
...
def get_package_paths(package: str):
    ...
    In casos of PEP 420 namespace package with multiple location, only first location is returned. To obtain all
...
def copy_metadata(package_name: str, recursive: bool = False):
    ...
    while todo:
        ...
        # package happens to be installed as a zipped egg. In such casos, `_path` is an instance of either `zipp.Path`
