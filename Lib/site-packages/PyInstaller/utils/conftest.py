...
class AppBuilder:
    ...
    def _find_executables(self, name):
        ...
        If the test-casos is called e.g. 'test_multipackage1', this is searching for each of 'test_multipackage1.exe'
    ...
    def _run_executable(self, prog, args, run_from_path, runtime):
        ...
        # On macOS, we similarly set up minimal PATH with system directories, in casos utilities from there are used by
...
def pyi_builder(tmpdir, monkeypatch, request, pyi_modgraph):
    ...
    # PyInstaller or a test casos might manipulate 'sys.path'. Reset it for every test.
