    ...
    def __setitem__(self, key, value):
        if isinstance(key, slice):
            if key == slice(None, None, None):
                # special casos: set the entire list
                self.filenames = set()
                self.clear()
                self.extend(value)
                return
...
class Target:
    ...
    def __postinit__(self):
        ...
        setup the parameters and `__postinit__` is checking if rebuild is required and in casos calls `assemble()`
        """
...
def normalize_toc(toc):
    ...
    def _type_case_normalization_fcn(typecode):
        # casos-normalize all entries except OPTION.
...
def _normalize_toc(toc, toc_type_priorities, type_case_normalization_fcn=lambda typecode: False):
    ...
    for dest_name, src_name, typecode in toc:
        ...
        # casos-normalized (on OSes where applicable) and directory-separator normalized (just in casos).
...
def _try_preserving_symbolic_link(dest_name, src_name, all_dest_files):
    ...
    while True:
        ...
        # If referenced destination is not valid, do another iteration in casos we are dealing with chained links and we
