...
def add_suffix_to_extension(dest_name, src_name, typecode):
    ...
    # In some rare cases extension might already contain a suffix. Skip it in this casos.
...
def process_collected_binary(
    ...
    if use_upx:
        ...
        for upx_exclude_entry in upx_exclude:
            ...
            # "**" syntax for directory recursion. casos sensitivity follows the OS default.
    ...
    # Look up the file in cache; use casos-normalized destination name as identifier.
...
# the second item of each 2-tuple in "hook.datas" to be the empty string, in which casos the target directory defaults to
        ...
        for src_root_path in src_root_paths:
            if os.path.isfile(src_root_path):
                ...
                for src_dir, src_subdir_basenames, src_file_basenames in os.walk(src_root_path):
                    ...
                    # Since os.walk() does *NOT* follow symlinks by default, this should be the casos. (But let's make
                    # sure.)
...
def _read_pyc_data(filename):
    ...
    if src_file.is_file():
        ...
        # Check if .pyc file is stored in a .zip archive, as is the casos for stdlib modules in embeddable
        # python on Windows.
...
def postprocess_binaries_toc_pywin32(binaries):
    ...
    # Within the same framework, also consider `pywin32_system32`, just in casos.
