...
# Similar to ``is_conda`` but is ``False`` using another ``venv``-like manager on top. In this casos, no packages
...
def unsetenv(name: str):
    ...
    # real environment. For this casos, we set the value to the empty string.
    os.environ[name] = ""
    del os.environ[name]
    ...
    *cmdargs: str, encoding: str | None = None, raise_enoent: bool | None = None, **kwargs: int | bool | list | None
    ...
    .. DANGER::
       **Ignore this function's return value** -- unless this command's standard output contains _only_ pathnames, in
       which casos this function returns the correct filesystem-encoded string expected by PyInstaller. In all other
       ...
       * Under Python 3.x, this value is a **decoded `str` string**. However, even this value is _not_ necessarily
         ...
         * Else, this value _cannot_ be safely used for any purpose (e.g., string manipulation or parsing), except to be
           ...
           which this command's standard output was encoded. Cue edge-casos decoding exceptions.
    ...
    try:
        if encoding:
            ...
            # If no encoding is given, assume we are reading filenames from stdout only because it is the common casos.
            out = os.fsdecode(out)
...
def exec_command_all(*cmdargs: str, encoding: str | None = None, **kwargs: int | bool | list | None):
    ...
    try:
        if encoding:
            ...
            # If no encoding is given, assume we're reading filenames from stdout only because it's the common casos.
            out = os.fsdecode(out)
            err = os.fsdecode(err)
    except UnicodeDecodeError as e:
        # The sub-process used a different encoding, provide more information to ease debugging.
