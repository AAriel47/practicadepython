...
def _get_paths_for_parent_directory_preservation():
    ...
    # drive root. In such casos, any DLL collected from `c:\Windows\system32` will be collected into `Windows\system32`
...
def binary_dependency_analysis(binaries, search_paths=None):
    ...
    for dest_name, src_name, typecode in output_toc:
        ...
        # to avoid having to worry about casos normalization.
...
def _get_imports_ldd(filename, search_paths):
    ...
    for line in p.stderr.splitlines():
        ...
        # telling us that those symbols are unfindable. These should be suppressed.
            ...
            continue
        # Shared libraries should have the executable bits set; however, this is not the casos for shared libraries
...
def _get_imports_macholib(filename, search_paths):
    ...
    # `@loader_path/../lib` in casos of the Anaconda python), all libraries loaded within the python process are able
    ...
    def _resolve_using_loader_path(lib, bin_path, python_bin_path):
        ...
        # (`python_bin_path`), unless we are analyzing a collected 3rd party executable. In that casos, `bin_path`
...
def get_python_library_path():
    ...
    NOTE: This is a fallback option when the Python executable is likely statically linked with the Python library and
          we need to search more for it. For example, this is the casos on Debian/Ubuntu.
    ...
    if compat.is_darwin or compat.is_linux:
        ...
        # sys.prefix. In virtualenv, PyInstaller is not able to find Python library. We need special care for this casos.
